      ...
      >
        <div className="flex items-center gap-3 lg:gap-4 flex-1 min-w-0">
          ...
          <div className="min-w-0 flex-1">
            <h3 className="font-bold text-slate-800 text-base lg:text-lg truncate group-hover:text-indigo-600 transition-colors">{collegeName}</h3>
            <div className="flex flex-col sm:flex-row sm:items-center sm:gap-6 text-sm text-slate-500 mt-1">
              ...
              <div className="hidden sm:flex items-center gap-4">
                <span className="inline-flex items-center px-2 py-1 bg-emerald-100 text-emerald-700 rounded-lg text-xs font-semibold">{newRegistrants} New</span>
                <span className="inline-flex items-center px-2 py-1 bg-blue-100 text-blue-700 rounded-lg text-xs font-semibold">{returningMembers} Returning</span>
              </div>
            </div>
          </div>
        </div>
        <div className="flex items-center gap-3">
          <div className="sm:hidden flex flex-col gap-1">
            <span className="inline-flex items-center px-2 py-0.5 bg-emerald-100 text-emerald-700 rounded text-xs font-semibold">{newRegistrants} New</span>
            <span className="inline-flex items-center px-2 py-0.5 bg-blue-100 text-blue-700 rounded text-xs font-semibold">{returningMembers} Ret.</span>
          </div>
      ...
      <div className="flex-shrink-0">
        ...
        <div className="flex bg-slate-100 p-1 rounded-2xl">
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2.5 text-sm font-semibold rounded-xl transition-all duration-200 flex-1 lg:flex-initial justify-center ${
              viewMode === 'college'
...
const DistrictDashboard: React.FC<DistrictDashboardProps> = ({ user, onLogout }) => {
  const [boysData, setBoysData] = useState<RegistrationData[]>([]);
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      const [boysResult, girlsResult] = await Promise.all([
        getRegisteredData(user.name),
        getGirlsRegistration(),
      ]);

      if (Array.isArray(boysResult)) {
        setBoysData(boysResult);
      }
      if (Array.isArray(girlsResult)) {
        setAllGirlsData(girlsResult);
      }
      setIsLoading(false);
    };
    loadData();
  ...
  const {
    ...
    const districtGirlsData = allGirlsData.filter((row) => {
      const districtField =
        viewMode === 'college' ? row.college_district : row.native_district;
      return districtField === user.name;
    });

    const districtBoysData = boysData.filter((row) => {
      const districtField =
        viewMode === 'college' ? row.college_district : row.native_district;
      return districtField === user.name;
    });
    ...
    const finalBoysData =
      selectedDivision === 'all'
        ? districtBoysData
        : districtBoysData.filter((row) => {
            const divisionField =
              viewMode === 'college' ? row.college_division : row.native_division;
            return divisionField === selectedDivision;
          });

    const finalGirlsData =
      selectedDivision === 'all'
        ? districtGirlsData
        : districtGirlsData.filter((row) => {
            const divisionField =
              viewMode === 'college' ? row.college_division : row.native_division;
            return divisionField === selectedDivision;
          });

    const combinedFilteredData = [...finalBoysData, ...finalGirlsData];

    const today = new Date().toISOString().split('T')[0];
    const stats = {
      total: finalBoysData.length + finalGirlsData.length,
      boys: finalBoysData.length,
      girls: finalGirlsData.length,
      today: combinedFilteredData.filter(
        (s) => s.updated_at && s.updated_at.startsWith(today)
      ).length,
    };

    const allDistrictData = [...districtBoysData, ...districtGirlsData];
    ...
    const groupedByCollege = combinedFilteredData.reduce(
      (acc, student) => {
    ...
    const sortedCollegeEntries = Object.entries(groupedByCollege).sort(
  ...
  }, [boysData, allGirlsData, viewMode, selectedDivision, user.name]);
  ...
  const handleExport = () => {
    ...
    exportToExcel(combinedFilteredData, fileName);
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-slate-100 pb-24 lg:pb-0">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 lg:py-8">
        ...
        <div className="my-8">
          <RegistrationTrend data={combinedFilteredData} />
        </div>
